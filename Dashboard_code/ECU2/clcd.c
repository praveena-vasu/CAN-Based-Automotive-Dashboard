...
void clcd_print(const unsigned char data[][3], unsigned char addr, unsigned char row)
{
	clcd_write(addr, INSTRUCTION_COMMAND);
    unsigned char i = 0;
	while (data[row][i] != '\0')
	{
		clcd_write(data[row][i++], DATA_COMMAND);
	}
}

void clcd_puts(const unsigned char *data, unsigned char addr)
{
	clcd_write(addr, INSTRUCTION_COMMAND);
	while (*data != '\0')
	{
